...
export default function DashboardView() {
  ...
  const [layout, setLayout] = React.useState<Array<{ id: string; visible: boolean; title: string }>>([
    { id: 'kpis', visible: true, title: 'Indicadores de Performance (KPIs)' },
    { id: 'revenue_evolution', visible: true, title: 'Evolução de Facturação' },
    { id: 'business_lines', visible: true, title: 'Vendas por Linhas de Negócio' },
    { id: 'projections', visible: true, title: 'Projecção vs Realizado' },
    { id: 'audit_logs', visible: true, title: 'Logs de Auditoria' },
  ]);

  React.useEffect(() => {
    const saved = localStorage.getItem(`ndf_dashboard_layout_${currentTenant?.id}`);
    if (saved) {
      try {
  ...
  const saveLayout = (newLayout: typeof layout) => {
    setLayout(newLayout);
    localStorage.setItem(`ndf_dashboard_layout_${currentTenant?.id}`, JSON.stringify(newLayout));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold transition-all border ${
              isCustomizing 
      ...
      {isCustomizing && (
        <div className={`p-4 rounded-xl border animate-in slide-in-from-top-2 duration-300 ${
          theme === 'dark' ? 'bg-slate-900/50 border-slate-800 text-slate-300' : 'bg-blue-50/50 border-blue-100 text-blue-800'
        }`}>
            ...
            {layout.map((item, idx) => (
              <div key={item.id} className={`flex items-center justify-between p-3 rounded-lg border ${
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => moveItem(idx, 'up')} disabled={idx === 0} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded disabled:opacity-20"><ChevronUp className="h-3.5 w-3.5" /></button>
                  <button onClick={() => moveItem(idx, 'down')} disabled={idx === layout.length - 1} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded disabled:opacity-20"><ChevronDown className="h-3.5 w-3.5" /></button>
                  <div className="w-px h-4 bg-slate-200 dark:bg-slate-800 mx-1" />
                  <button 
                    onClick={() => toggleVisibility(item.id)}
                    className={`p-1.5 rounded-md transition-colors ${item.visible ? 'text-blue-500 bg-blue-500/10' : 'text-slate-400 bg-slate-100 dark:bg-slate-800'}`}
                  >
      ...
      <div className="space-y-8">
        {layout.map((item) => {
          ...
          switch (item.id) {
            case 'kpis':
              return (
                <div key="kpis" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            case 'business_lines':
              return (
                <div key="business_lines" className={`p-6 rounded-xl border ${
                  theme === 'dark' ? 'bg-slate-950 border-slate-900' : 'bg-white border-slate-200'
                } flex flex-col justify-between max-w-2xl`}>
                  <div>
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-8">
                    <div className="h-48 w-48 flex items-center justify-center py-4">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie
                    ...
                    <div className="flex-1 space-y-2 text-xs text-slate-400">
                      {(stats.categorySales || []).map((cat, idx) => (
                        <div key={cat.category} className="flex justify-between items-center gap-4">
                          <div className="flex items-center gap-2">
            ...
            case 'projections':
              return (
                <div key="projections" className={`p-6 rounded-xl border ${
                  theme === 'dark' ? 'bg-slate-950 border-slate-900' : 'bg-white border-slate-200'
                }`}>
                  <div className="flex justify-between items-center mb-6">
                    <div>
            ...
            case 'audit_logs':
              return (
                <div key="audit_logs" className={`p-6 rounded-xl border ${
                  theme === 'dark' ? 'bg-slate-950 border-slate-900' : 'bg-white border-slate-200'
                }`}>
